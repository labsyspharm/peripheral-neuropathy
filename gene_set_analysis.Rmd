...
de_long_batch2 <- syn("syn69968964") %>%
  read_csv() %>%
...
feasible_genes_batch2 <- syn("syn70094663") %>%
  read_csv()
```
...
de_batch2_selected <- de_long_batch2 %>%
  filter(
...
de_long_selected_both <- bind_rows(
  batch1 = de_long_selected,
  batch2 = de_batch2_selected,
  .id = "batch"
)
...
## Plotting batch 1 vs 2

```{r}
de_long_selected_vs <- de_long_selected_both %>%
  select(batch, gene_id, gene_name, contrast, logFC, FDR, signed_padj) %>%
  pivot_wider(
    names_from = batch,
    values_from = c(logFC, FDR, signed_padj),
    names_sep = "_"
  )
...
p <- ggplot(
  ...
  facet_wrap(
    ~contrast, scales = "free"
  )
p
ggsave(
  file.path("plots", "batch1_vs_batch2_signed_padj_scatter.pdf"),
...
feasible_genes_batch1 <- raw_counts_long %>%
  group_by(gene_id) %>%
  summarize(
...
feasible_genes <- bind_rows(
  batch1 = feasible_genes_batch1,
  batch2 = feasible_genes_batch2,
  .id = "batch"
)
...
feasible_genes <- read_csv("feasible_genes.csv.gz")
```
...
de_meta <- raw_counts_long %>%
  ...
  mutate(
    paclitaxel = if_else(str_detect(condition, "PTX"), "PTX", "DMSO"),
    treatment = case_when(
      str_detect(condition, "GNE495") ~ "GNE-495",
      str_detect(condition, "JNKinh") ~ "JNK-inh",
      TRUE ~ "DMSO"
    )
  )
...
de_all_ruvk4 <- DESeq2::DESeqDataSetFromMatrix(
  raw_counts_ruvk4 %>%
    select(gene_id, sample_id, count) %>%
    pivot_wider(names_from = sample_id, values_from = count) %>%
    # S36 missing for some reason...
    select(gene_id, any_of(de_meta$sample_id)) %>%
    column_to_rownames("gene_id") %>%
    as.matrix(),
  de_meta %>%
    filter(sample_id %in% raw_counts_ruvk4$sample_id),
  design = ~ condition
) %>%
...
```{r}
counts_trans_ruvk4 <- list(
  vsn = SummarizedExperiment::assay(DESeq2::varianceStabilizingTransformation(de_all_ruvk4, blind = FALSE)),
  rlog = SummarizedExperiment::assay(DESeq2::rlog(de_all_ruvk4, blind = FALSE)),
  normalized = DESeq2::counts(de_all_ruvk4, normalized = TRUE),
  raw = DESeq2::counts(de_all_ruvk4, normalized = FALSE)
) %>%
  map(
...
de_long_selected %>%
  compute_point_density("logFC", "signed_padj")

picked_ptx_vs_dmso_genes <- c(
  ...
  mutate(
    p = pmap(
      ...
      \(contrast, data, ...) {
        p_data <- data %>%
          mutate(
            nlog10_FDR = -log10(FDR)
          ) %>%
          filter(is.finite(nlog10_FDR)) %>%
          compute_point_density("logFC", "nlog10_FDR", n = 100)

        # Use manually selected genes for PTX
        if (contrast == "PTX") {
          p_data <- p_data %>%
            mutate(
              gene_name_repel = if_else(
                # .density < quantile(.density, .002) |
                gene_name %in% picked_ptx_vs_dmso_genes,
                gene_name,
                ""
              )
            )
        ...
        ggplot(
          ...
          geom_text_repel(
            ...
            #     color = gene_name %in% picked_ptx_vs_dmso_genes
            #   )
            # else
            aes(
              label = gene_name_repel
...
pwalk(
  ...
  \(contrast, p, ...) {
    ggsave(
      file.path("plots", paste0("volcano_", contrast, "_raster.pdf")),
      p, width = 6, height = 4
    )
  }
)
...
htmlwidgets::saveWidget(
  x,
  file = "plots/volcano_PT_vs_DMSO.html",
  selfcontained = FALSE
)
```
...
ptx_rescue_list_neat <- ptx_rescue_list %>%
  select(
    batch, gene_id, compound, perturbed,
    starts_with("rescue")
  )
...
ptx_rescue_gene_sets_short <- bind_rows(
  ptx_rescue_gene_sets %>%
    dplyr::select(
      batch, gene_id, compound, perturbed, return_baseline, significant_opposite, overshot
    ) %>%
    filter(perturbed != "not perturbed") %>%
    pivot_longer(
      -c(batch, gene_id, compound, perturbed),
      names_to = "rescue_definition",
      values_to = "rescue"
    ) %>%
    filter(rescue == "yes"),
...
```{r}
### Have to fix the `complete()` statement below to include all genes in
...
ptx_rescue_gene_sets_list <- ptx_rescue_gene_sets_short %>%
  group_by(
    batch, compound, perturbed, rescue_definition
  ) %>%
  summarize(
    gene_vec = list(
      {
...
topgo_ptx_rescue_type_list <- ptx_rescue_type_list %>%
  group_by(
    batch, perturbed, rescue_definition, rescue_type
  ) %>%
  summarize(
    gene_vec = list(
      set_names(
...
ptx_rescue_overrepresentation <- ptx_rescue_overrepresentation_raw %>%
  transmute(
    batch, rescue_definition, perturbed, compound,
    res = map2(
      res, res_fisher,
      \(x, y) power_full_join(
...
ptx_rescue_intersections <- ptx_rescue_type_list %>%
  filter(perturbed != "not perturbed") %>%
  group_nest(batch, rescue_definition, perturbed, rescue_type) %>%
  bind_rows(
    ptx_rescue_type_list %>%
      filter(perturbed != "not perturbed") %>%
      group_nest(batch, rescue_definition, rescue_type) %>%
      mutate(
...
ptx_rescue_intersections_overrepresentation <- ptx_rescue_intersections_overrepresentation_raw %>%
  transmute(
    batch, rescue_definition, perturbed, rescue_type,
    res = map2(
      res, res_fisher,
      \(x, y) power_full_join(
        dplyr::rename(x, p.value = pval),
...
ptx_rescue_alluvial_plots <- ptx_rescue_type_list %>%
  group_nest(batch, rescue_definition) %>%
  mutate(
    alluvial_data = map(
      data,
      \(x) count(
...
pwalk(
  ...
  \(p, batch, rescue_definition, ...) ggsave(
    file.path("plots", paste0("ptx_rescue_", rescue_definition, "_alluvium_", batch, ".pdf")),
    p, width = 6, height = 4
  )
)
