...
fgsea_rescue_vectors <- ptx_rescue_by_compound %>%
  ...
  mutate(
    signed_padj_rescue_raw = if_else(
      !xor(signed_padj > 0, `signed_padj + PTX vs PTX` > 0),
      0,
      `signed_padj + PTX vs PTX`
    ),
    logFC_rescue_raw = if_else(
      !xor(logFC > 0, `logFC + PTX vs PTX` > 0),
      0,
      `logFC + PTX vs PTX`
    ),
...
topgo_res_plotting_clust <- topgo_res_plotting %>%
  dplyr::transmute(ontology, term_unique, rescue_type, across(fisher_double, \(x) -log10(x))) %>%
  group_nest(ontology) %>%
  mutate(
    data = map(
      data,
      \(x) cluster_df(x, term_unique, rescue_type, fisher_double)
    )
  ) %>%
  with(set_names(data, ontology))

plot_big_heatmap <- \(x, df) ggplot(
  df %>%
    mutate(
...
ps <- map(
  names(topgo_res_plotting_clust) %>%
    set_names(),
  \(x, y) plot_big_heatmap(x, topgo_res_plotting)
)
...
iwalk(
  ...
  \(p, name) {
    ggsave(
      file.path("plots", paste0("topgo_big_heatmap_", name, "2.pdf")),
...
ps <- map(
  names(topgo_res_plotting_clust) %>%
    set_names(),
  \(x, y) plot_big_heatmap(x, topgo_res_plotting_picks)
)
