...
row_clust <- cluster_fun_eucl(hm_mat_scaled, sample_in_col = TRUE)
library(impute)
row_clust_k <- kmeans(impute.knn(hm_mat_scaled)$data, centers = 15, nstart = 10) %>%
  broom::augment(data = tibble(gene_id = rownames(hm_mat_scaled)))
...
withr::with_cairo_pdf(
  file.path("plots", "de_heatmaps", "de_heatmap_all_combined_signed_p_scaled_smaller.pdf"),
  draw(hm),
  width = 5, height = 14
)
...
de_sheet <- de %>%
  ...
  transmute(
    gene_id,
    name_display,
    log2FC = log2FoldChange, padj, signed_p
  ) %>%
  # Scale by gene
  group_by(gene_id) %>%
  mutate(across(starts_with("signed_p"), \(x) scale(x, center = FALSE, scale = TRUE)[, 1])) %>%
  ungroup() %>%
  pivot_wider(
    names_from = name_display,
    values_from = c(log2FC, padj, signed_p),
    names_glue = "{name_display}_{.value}",
    names_vary = "slowest"
  ) %>%
  power_left_join(
    gene_info,
  ...
  relocate(
    gene_id, gene_name, gene_description, gene_biotype, chromosome, .cluster,
    .before = 1
  ) %>%
  slice(
    match(
      rownames(hm_mat_scaled)[unlist(row_order(hm))],
      gene_id
    )
  )
...
addWorksheet(de_table_wb, "DE clusters")
writeDataTable(de_table_wb, "DE clusters", de_sheet, tableStyle = "TableStyleLight8")
walk(
  which(str_detect(names(de_sheet), coll("padj"))),
  \(i) {
...
walk(
  which(str_detect(names(de_sheet), "log2FC|signed_p")),
  \(i) {
    data_rows <- nrow(de_sheet) + 1L  # +1 for header
...
setColWidths(de_table_wb, "DE clusters", cols = seq_len(ncol(de_sheet)), widths = "auto")

saveWorkbook(de_table_wb, file.path("tables", "de_clustering.xlsx"), overwrite = TRUE)
