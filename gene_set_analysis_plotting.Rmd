...
ps <- fgsea_intersection_overrep_data %>%
  ...
  group_nest(
    batch, rescue_type, rescue_definition
  ) %>%
  mutate(
    p = pmap(
...
pwalk(
  ps,
  \(batch, rescue_definition, rescue_type, p, ...) {
    ggsave(
      file.path("plots", "overrep", paste0("overrep_intersection_", batch, "_", rescue_definition, "_", rescue_type, "_bars.pdf")),
      p, width = 8, height = 8
    )
  }
)
...
fgsea_intersection_overrep_data_single <- fgsea_intersection_overrep_data %>%
  ...
  group_by(
    batch, rescue_definition, rescue_type, database, pathway_db, pathway
  ) %>%
  summarize(
    direction = case_when(
